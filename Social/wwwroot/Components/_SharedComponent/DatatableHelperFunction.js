...
function CreateCommonDatatableAjax(DatatableConfigration, initCompletefunc = "", UseTableCounter = true, UseDefaultAction = true, drawCallbackfunc = "", checkBoxColumn = false) {
        ...
        DatatableConfigration.Columns = [

            {
                ...
                'render': function (data, type, full, meta) {
                    console.log(full)
                    return `<input type="checkbox" name="id[${full.EntityId}]" onChange=" DataChange('${full.EntityId}',event.target.checked)">`;
                }
            },
            {
                "data": "ID",
    ...
    if (!$.fn.DataTable.isDataTable(DatatableConfigration.TableID)) {
        debugger;
        table = $(DatatableConfigration.TableID).DataTable({
            paging: true,
            serverSide: true,
            processing: true,
            //scrollY: 300,
