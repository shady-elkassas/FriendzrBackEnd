...
namespace Social.Services.Implementation
{
    public class MessageServes : IMessageServes
    {
            ...
            {
                {
                    ...
                    if (a != 0)
                    {
                        ...
                        if(relation != null && user.IsWhiteLabel.HasValue && user.IsWhiteLabel.Value)
                        {
                            ...
                            data.Image = (item.FirstOrDefault().UserMessagess.ToUserId == currentuserid ? item.FirstOrDefault().UserMessagess.User.UserImage : item.FirstOrDefault().UserMessagess.ToUser.UserImage);
                            data.id = id;
        ...
        {
            var messdata = _authContext.UserMessages.Where(e => e.ToUser.UserId == userid && e.ToUserNotreadcount > 0).ToList();
            var tomes = messdata.Sum(m => m.ToUserNotreadcount);
